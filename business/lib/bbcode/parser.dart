  ...
  while (true) {
    for (Match match in _bbcodeTagRegExp.allMatches(tail)) {
      ...
      if (match[0] == '[img]' &&
        ...
        String stickerName =
            imageUrlToStickerName(tail.substring(match.end, end));
        if (stickerName != null) {
          yield StickerTag(stickerNameToFilename[stickerName]);
        } else {
      ...
      if (match[1].startsWith('s:') &&
        ...
        yield StickerTag(stickerNameToFilename[match[1].substring(2)]);
